...
class Calendar_IO:
    ...
    def add_to_calendar(self, event, calendar):
        creds = self.get_credentials()
        service = discovery.build("calendar", "v3", credentials=creds)
        start_time = datetime.strptime("2024 " + event["time"] + " -0400",
                                       "%Y %a, %b %d, %H:%M %z")  # Sat, Mar 16, 11:00
        end_time = start_time + timedelta(hours=2)

        event_start = start_time.strftime("%Y-%m-%dT%H:%M:00%z")  # Sat, Mar 16, 11:00
    ...
    def test_calendar(self):
        ...
        event = {"players": "P1-P2", "time": "Sat, Mar 16, 11:00", "round": "1", "board": "3", "division": "TestDivision"}
        this_io = Calendar_IO(
